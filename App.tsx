      ...
      if (e.target.files && e.target.files.length > 0) {
          ...
          setTimeout(() => {
              ...
              setUserImages(prev => {
                  ...
                  // Limit to 60 photos to maintain performance
                  if (combined.length > 60) {
                      const toRemove = combined.slice(0, combined.length - 60);
                      const toKeep = combined.slice(combined.length - 60);
                      
                      // Revoke old URLs to prevent memory leaks
                      toRemove.forEach(url => URL.revokeObjectURL(url));
                      return toKeep;
                  }
                  return combined;
              });
